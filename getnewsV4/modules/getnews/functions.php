...
$global_array_cat[0] = array("catid" => 0, "parentid" => 0, "title" => "Other",
    "titlesite" => "", "alias" => "Other", "link" => $link_i, "viewcat" =>
    "viewcat_page_new", "subcatid" => 0, "numlinks" => 3, "description" => "",
    "inhome" => 0, "keywords" => "");
$catid = 0;
$parentid = 0;
$alias_cat_url = isset($array_op[0]) ? $array_op[0] : "";
$array_mod_title = array();
$sql = "SELECT catid, parentid, title, titlesite, alias, viewcat, subcatid, numlinks, description, inhome, keywords, who_view, groups_view FROM `" .
...
foreach ($list as $l)
{
    ...
    $global_array_cat[$l['catid']]['link'] = NV_BASE_SITEURL . "index.php?" .
        NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name .
...
if ($module_info['rss'])
{
    $rss[] = array('title' => $module_info['custom_title'], 'src' => NV_BASE_SITEURL .
        "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" .
        NV_NAME_VARIABLE . "=news&amp;" . NV_OP_VARIABLE . "=rss");
}
foreach ($global_array_cat as $catid_i => $array_cat_i)
{
    if ($catid_i > 0 and $array_cat_i['parentid'] == 0)
    {
        ...
        $nv_vertical_menu[] = array($array_cat_i['title'], $array_cat_i['link'], $act,
            'submenu' => $submenu);
    }
    if ($catid_i and $module_info['rss'])
    {
        $rss[] = array('title' => $module_info['custom_title'] . ' - ' . $array_cat_i['title'],
            'src' => NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA .
            "&amp;" . NV_NAME_VARIABLE . "=news&amp;" . NV_OP_VARIABLE .
            "=rss/" . $array_cat_i['alias']);
    }
}
...
if (!empty($array_op) and $op == "main")
{
    if ($catid == 0)
    {
        ...
        {
            $redirect = "<meta http-equiv=\"Refresh\" content=\"3;URL=" . nv_url_rewrite(NV_BASE_SITEURL .
                "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" .
                NV_NAME_VARIABLE . "=news", true) . "\" />";
    ...
    } else
    {
        ...
        while ($parentid > 0)
        {
            ...
            $array_mod_title[] = array('catid' => $parentid, 'title' => $array_cat_i['title'],
