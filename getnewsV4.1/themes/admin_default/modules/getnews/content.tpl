...
<!-- END: error -->
<form id="getcontent" method="post" action="">
    <table summary="" class="tab1">
    <caption>{LANG.linkbaiviet}</caption>
		<tbody>
			<tr>
				<td align="right"><strong>{LANG.linksite}: </strong></td>
				<td><input style="width: 650px" name="link_article" type="text" value="{site}" maxlength="255" /></td>
                <td width="250"><input type="submit" name="laytin" value="{LANG.laytin}"/></td>
			</tr>
		</tbody>
	</table>
</form>
	...
	<div class="gray">
		<table width="100%" style="margin-bottom:0">
			<tr>
				<td valign="top">
				<table summary="" class="tab1">
                <caption>{LANG.noidungbaiviet}</caption>
					<tbody>
						<tr>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
        $("#checkalias").click(function(){
            var title = $("#idtitle").val();
             if (title != '') {
        	    $.ajax({
        	        url: script_name + "?"+ nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=alias',
        			type: "POST",
        			cache: false,
        			data: "title="+title,
	...
	if (title != '') {
		nv_ajax('post', script_name, nv_name_variable + '=' + nv_module_name + '&' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=alias&title=' + encodeURIComponent(title), '', '');
	}
	return false;
    }
    function res_get_alias(res) {alert(res);
    	if (res != "") {
    		document.getElementById('idalias').value = res;
    	} else {
    		document.getElementById('idalias').value = '';
