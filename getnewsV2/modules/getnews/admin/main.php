...
{
    Header( "Location: " . NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=news&" . NV_OP_VARIABLE . "=cat" );
    die();
}
...
$array_search = array("-" => "---", "title" => $lang_module['search_title'],
    "bodytext" => $lang_module['search_bodytext'], "author" => $lang_module['search_author'],
    "admin_id" => $lang_module['search_admin'], "sourcetext" => $lang_module['sources']);
...
if ($catid == 0)
{
    $from = "`" . NV_PREFIXLANG . "_news_rows` as r";
} else
{
    $from = "`" . NV_PREFIXLANG . "_news_" . $catid . "` as r";
}
$where = "";
...
if ($checkss == md5(session_id()) and !empty($q))
{
    if ($stype == "bodytext")
    {
        $from .= " INNER JOIN `" . NV_PREFIXLANG . "_news_bodytext` as c ON (r.id=c.id)";
    ...
    } elseif ($stype == "sourcetext")
    {
        ...
        $where = " WHERE r.\tsourceid IN (SELECT `sourceid` FROM `" . NV_PREFIXLANG .
            "_news_sources` WHERE `title` like '%" . $db->dblikeescape($q) .
            "%' OR `link` like '%" . $db->dblikeescape($qurl) . "%')";
    } elseif ($stype == "admin_id")
    {
        $where = " WHERE u.username LIKE '%" . $db->dblikeescape($qhtml) .
    ...
    } elseif (!empty($q))
    {
        ...
        $where = " WHERE r.author LIKE '%" . $db->dblikeescape($qhtml) . "%' \n\t\t\tOR r.title LIKE '%" .
            $db->dblikeescape($qhtml) . "%' \n\t\t\tOR c.bodytext LIKE '%" . $db->
...
if (!defined('NV_IS_ADMIN_MODULE'))
{
    ...
    $where .= (empty($where)) ? " WHERE (" . implode(" OR ", $from_catid) . ")" :
        " AND (" . implode(" OR ", $from_catid) . ")";
}
...
{
    $search_type[] = array("key" => $key, "value" => $val, "selected" => ($key == $stype) ?
        " selected=\"selected\"" : "");
}
$i = 5;
...
while ($i <= 1000)
{
    $search_per_page[] = array("page" => $i, "selected" => ($i == $per_page) ?
        " selected=\"selected\"" : "");
    $i = $i + 5;
}
$a = 0;
$order2 = ($order == "asc") ? "desc" : "asc";
$base_url_id = "" . NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name .
    "&amp;" . NV_OP_VARIABLE . "=" . $op . "&amp;per_page=" . $per_page .
    "&amp;catid=" . $catid . "&amp;stype=" . $stype . "&amp;q=" . $q .
    "&amp;checkss=" . $checkss . "&amp;ordername=id&amp;order=" . $order2 .
    "&amp;page=" . $page;
$base_url_name = "" . NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" .
    $module_name . "&amp;" . NV_OP_VARIABLE . "=" . $op . "&amp;per_page=" . $per_page .
    "&amp;catid=" . $catid . "&amp;stype=" . $stype . "&amp;q=" . $q .
    "&amp;checkss=" . $checkss . "&amp;ordername=title&amp;order=" . $order2 .
    "&amp;page=" . $page;
$base_url_publtime = "" . NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE .
    "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=" . $op . "&amp;per_page=" . $per_page .
    "&amp;catid=" . $catid . "&amp;stype=" . $stype . "&amp;q=" . $q .
    "&amp;checkss=" . $checkss . "&amp;ordername=publtime&amp;order=" . $order2 .
    "&amp;page=" . $page;
$base_url_exptime = "" . NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE .
    "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=" . $op . "&amp;per_page=" . $per_page .
    "&amp;catid=" . $catid . "&amp;stype=" . $stype . "&amp;q=" . $q .
    "&amp;checkss=" . $checkss . "&amp;ordername=exptime&amp;order=" . $order2 .
    "&amp;page=" . $page;
$base_url = "" . NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name .
    "&amp;" . NV_OP_VARIABLE . "=" . $op . "&amp;per_page=" . $per_page .
    "&amp;catid=" . $catid . "&amp;stype=" . $stype . "&amp;q=" . $q .
    "&amp;checkss=" . $checkss . "&amp;ordername=" . $ordername . "&amp;order=" . $order;
$ord_sql = "ORDER BY r." . $ordername . " " . $order;
$sql = "SELECT SQL_CALC_FOUND_ROWS r.id, r.catid, r.listcatid, r.admin_id, r.title, r.alias, r.status , r.publtime, r.exptime, u.username  FROM " .
    $from . " " . $where . " " . $ord_sql . " LIMIT " . $page . "," . $per_page;

$result = $db->sql_query($sql);
$result_all = $db->sql_query("SELECT FOUND_ROWS()");
list($all_page) = $db->sql_fetchrow($result_all);
$data = array();$z="sdfs";
while (list($id, $catid_i, $listcatid, $post_id, $title, $alias, $status, $publtime,
    $exptime, $username) = $db->sql_fetchrow($result))
{
    ...
    $data[] = array("class" => $class, "id" => $id, "link" => $link, "title" => $title,
        "publtime" => $publtime, "status" => $lang_module['status_' . $status],
        "username" => $username, "feature" => implode("&nbsp;-&nbsp;", $admin_funcs));
    ++$a;
}
$array_list_action = array('delete' => $lang_global['delete'], 'publtime' => $lang_module['publtime'],
