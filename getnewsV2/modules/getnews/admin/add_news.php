...
$array_structure_image['username_Y'] = $module_name . '/' . $username_alias .
    '/' . date('Y');
$array_structure_image['username_Ym'] = $module_name . '/' . $username_alias .
    '/' . date('Y_m');
$array_structure_image['username_Y_m'] = $module_name . '/' . $username_alias .
    '/' . date('Y/m');
$array_structure_image['username_Ym_d'] = $module_name . '/' . $username_alias .
    '/' . date('Y_m/d');
$array_structure_image['username_Y_m_d'] = $module_name . '/' . $username_alias .
    '/' . date('Y/m/d');
$structure_upload = isset($module_config[$module_name]['structure_upload']) ? $module_config[$module_name]['structure_upload'] :
    "Ym";
...
$rowcontent = array("id" => "", "catid" => $catid, "listcatid" => "" . $catid .
    ...
    1, "bodyhtml" => "", "copyright" => 0, "inhome" => 1, "allowed_comm" => $module_config[$module_name]['setcomm'],
...
if ($nv_Request->get_int('save', 'post') == 1)
{
    ...
    if (empty($error))
    {
        ...
        if ($check_thumb and file_exists($homeimgfile))
        {
            ...
            $i = 1;
            while (file_exists(NV_ROOTDIR . '/' . NV_FILES_DIR . '/' . $module_name .
            ...
            $thumb_name = str_replace(NV_ROOTDIR . '/' . NV_FILES_DIR . '/' . $module_name .
                '/', '', $image_info['src']);
            $block_basename = $basename;
            $i = 1;
            while (file_exists(NV_ROOTDIR . '/' . NV_FILES_DIR . '/' . $module_name .
                '/block/' . $block_basename))
            {
            ...
            $image_info = $image->create_Image_info;
            $block_name = str_replace(NV_ROOTDIR . '/' . NV_FILES_DIR . '/' . $module_name .
                '/', '', $image_info['src']);
            $image->close();
        ...
        if ($rowcontent['id'] == 0)
        {
            ...
            if ($rowcontent['id'] > 0)
            {
                ...
                {
                    $ct_query[] = (int)$db->sql_query("INSERT INTO `" . NV_PREFIXLANG . "_news_" . $catid . "` SELECT * FROM `" . NV_PREFIXLANG . "_news_rows` WHERE `id`=" . $rowcontent['id'] . "");
                }
        ...
        } else
        {
            ...
            if ($db->sql_affectedrows() > 0)
            {
                ...
                $ct_query[] = (int)$db->sql_query("UPDATE `" . NV_PREFIXLANG . "_news_bodytext` SET `bodytext`=" . $db->dbescape_string($rowcontent['bodytext']) .
                    " WHERE `id` =" . $rowcontent['id']);
                if (array_sum($ct_query) != sizeof($ct_query))
                {
                    $error[] = $lang_module['errorsave'];
...
//            {
//                $db->sql_query("INSERT INTO `" . NV_PREFIXLANG . "_" . $module_data .
//                    "_block` (`bid`, `id`, `weight`) VALUES ('" . $bid_i . "', '" . $rowcontent['id'] .
//                    "', '0')");
//            }
//            $id_block_content[] = 0;
//            $db->sql_query("DELETE FROM `" . NV_PREFIXLANG . "_" . $module_data .
//                "_block` WHERE `id` = " . $rowcontent['id'] . " AND `bid` NOT IN (" . implode(",",
//                $id_block_content) . ")");
...
if (!empty($rowcontent['homeimgfile']) and file_exists(NV_UPLOADS_REAL_DIR . "/" .
    $module_name . "/" . $rowcontent['homeimgfile']))
{
    $rowcontent['homeimgfile'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . "/" . $module_name .
        "/" . $rowcontent['homeimgfile'];
}
...
echo nv_admin_theme(change_alias("cuong sdajf"));
