...
class GetContent
{
    ...
    static function SplitAuto($b,$tag,&$Title,&$head,&$conten,$pos,$root)
    {
        ...
        $noidung=GetContent::CheckNode($root,$conten,$maxword);//echo $noidung;
...
class GetArea
{
    static function GetContent($link)
    {
     ...
     {
        $tag=array("h1","h2","p");$test=$b;
        foreach($tag as $t)
        {
            //tim toi node chua cac the trong mang tag
            $find=$b->find($t,0);
            //Neu tim thay
            if(!is_null($find))
            {
                //dem so tu trong node do
                $totalWord=GetContent::CountTotalWord($find->plaintext);
                //bien kq de luu so node cha cua node find tim dc
                $kq=0;
                //bien vt de luu vi tri cua node find so voi cac node anh em co cung cha voi node find
                $vt=0;
                while(!is_null($find->previousSibling()))
                {
                    //tang bien vt len 1
                    $vt=$vt+1;
                    $find=$find->previousSibling();
                }
                GetArea::C($find,$totalWord,$kq,$vt);
                //dua ra dc xpath tu node tim duoc chua the tag toi node goc truyen vao
                for($i=0;$i<$kq;$i++)
                {
                    $b=$find->parent();
                    
                }
                //tim xpath toi node goc
                $temp=$b->parent();
                $nodepatent=$temp->parent();
                $xpath=array();
                while($nodepatent && $nodepatent!=$test->parent())
                {
                    $stt=0;
                    while(!is_null($temp->previousSibling()))
                    {
                ...
                for($tam=count($xpath)-1;$tam>=0;$tam--)
                {
                    $pos[]=$xpath[$tam];
                }
     ...
     //Dua ra node co chua nhieu tu nhat va chua the tieu de tim dc o trong mang tag tren
     static function C(&$node,$totalword,&$kq,$vt)
     {
        //Neu la node goc thi thoat
        if(is_null($node->parent())) exit;
        //Lay so node anh em cua node dang xet
        $child=$node->parent()->childNodes;
        //tang bien kq len 1 tuc la tren node dang xet co it nhat 1 cha
        $kq=$kq+1;
        $count=0;
        //Lấy số lượng các node anh em của node truyền vào
        if(count($child)>0)
        {
            for($i=0;$i<count($child);$i++)
            {
                //bo qua node find tim thay
                if($i==$vt)continue;
                $nextNode=$node->nextSibling();
                //dem tong so tu co trong node cha ca node dang xet
                $count=$count+GetContent::CountTotalWord($nextNode->plaintext);
            }
        }
        
        //Neu trong node cha do ma co tong so tu nho hon tong so tu cua node truyen vao do phan noi dung bao gio cung
        //phai chua nhieu thong tin hon phan tieu de
        if($count<$totalword)
        {
            //Neu ma node cha dang xet nay co cha thi goi lai ham
            if(!is_null($node->parent()))
            {
                GetArea::C($node->parent(),$count,$kq,-1);
            }
        }
     } 
}
